...
export default function DashboardPage() {
  ...
  const continueCourse = stats?.myCourses?.courses?.find(
    (course) => course.status === 'in_progress' || course.status === 'assigned',
  );
  const continueThumbnailUrl = resolveMediaUrl(continueCourse?.courseThumbnail);
                      ...
                      <div>
                        <div className="mb-1 flex justify-between text-xs">
                          <span className="font-semibold text-slate-700">
                            {continueCourse.status === 'assigned' ? 'Sẵn sàng học' : 'Tiến độ'}
                          </span>
